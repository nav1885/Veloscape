...
| **In-Ride Single-Glance + Lock-Screen Controls Amendment** | `docs/amendments/InRideSingleGlanceAndLockScreenControls_Amendment.md` | Strips the in-ride segment roster → single-glance (ActiveCard only while in a segment; between segments = elapsed/distance/"N of M"/coaching indicator + Mute/End). Promotes lock-screen Mute/Unmute + End to core scope via `expo-notifications` (new dep). Adds ride-scoped `cuesMuted` to rideStore gating ALL cue sites in `shouldFireCue` (not just Recovery's). Backgrounded End = silent stop+save → summary on next open (`summary_viewed` column + `endRideAndSave()` refactor). Supersedes the "InRideScreen visuals unchanged" line in PhoneAsCoach. |
